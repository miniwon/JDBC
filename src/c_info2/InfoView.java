...
//	JLabel lbName, lbID, lbTel, lbGender, lbAge, lbHome;
	...
	JTextArea ta;
	JButton bAdd, bShow, bSearch, bDelete, bCancel, bUpdate;
...
//		}
	...
	void deleteByTel() {
		...
		try {
			int result = model.deleteByTel(tel);
		
		// (3) 화면을 지우고
			clearText();
			ta.setText("삭제한 행 수: " + result);
		} catch (Exception e) {
			ta.setText("전화번호 삭제 실패: " + e.getMessage());
		}
	...
	void updateByTel() {
		...
		// (3) 모델의 updateByTel() 호출
		try {
			model.updateByTel(vo);
		} catch (SQLException e) {
			e.printStackTrace();
		}
